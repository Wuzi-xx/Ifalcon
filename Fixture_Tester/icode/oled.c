...
void OLED_ShowFloat(uint8_t x, uint8_t y, float num, uint8_t z_len, uint8_t f_len, uint8_t size2, uint8_t Color_Turn)
{
    ...
    int frac_part = (int)((num - int_part) * oled_pow(10, f_len)); // 小数部分
    uint8_t start_x = x;

    // 负数处理
    if (num < 0) {
        OLED_ShowChar(start_x, y, '-', size2, Color_Turn);
        start_x += size2 / 2;
        int_part = -int_part;
        frac_part = -frac_part;
    }
    ...
    // 整数部分显示
    if (int_part >= 10) {                     // 两位整数
        OLED_ShowChar(start_x, y, (int_part / 10) + '0', size2, Color_Turn);
        OLED_ShowChar(start_x + size2 / 2, y, (int_part % 10) + '0', size2, Color_Turn);
        start_x += size2;
    } else {                                  // 一位整数，不显示前导零
        OLED_ShowChar(start_x, y, int_part + '0', size2, Color_Turn);
        start_x += size2 / 2;
    }
    ...
    if (f_len > 0) {
        for (i = f_len - 1; i >= 0; i--) {
            OLED_ShowChar(start_x + (f_len - 1 - i) * (size2 / 2), y,
                           (frac_part / oled_pow(10, i)) % 10 + '0',
                           size2, Color_Turn);
        }
    }
}
...
/**
 * @function: void OLED_DrawPoint(uint8_t x, uint8_t y, uint8_t color)
 * @description: 直接在指定位置画点（非缓冲区方案），color: 1=亮，0=灭
 * @param x: 0~127
 * @param y: 0~63
 */
void OLED_DrawPoint(uint8_t x, uint8_t y, uint8_t color)
{
    ...
    // 构造字节数据（SSD1306每页8行，bit 0 在最下）
    data = (1 << (y % 8));
    if (color) {
        OLED_WR_DATA(data);  // 亮点
    } else {
        OLED_WR_DATA(0x00);  // 清除该位置（此方法简单但可能覆盖其他点）
    }
}
